/*
  # Create Complete Listings System for HoyConnect

  ## Overview
  This migration creates a comprehensive listing system supporting hotels and guesthouses.
  ...
  All tables have proper RLS enabled with appropriate policies for hosts, guests, and admins.
*/
...
CREATE POLICY "Hosts and admins can view own listings"
...
WITH CHECK (
  ...
  EXISTS (SELECT 1 FROM profiles WHERE id = (select auth.uid()) AND role IN ('host', 'admin', 'super_admin'))
);
...
CREATE POLICY "Hosts and admins can view hotel details"
ON hotels FOR SELECT
TO authenticated
USING (
  EXISTS (SELECT 1 FROM listings WHERE id = listing_id AND (host_id = (select auth.uid()) OR EXISTS (SELECT 1 FROM profiles WHERE id = (select auth.uid()) AND role IN ('admin', 'super_admin'))))
);
...
CREATE POLICY "Hosts and admins can view rooms"
ON rooms FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM hotels h
    JOIN listings l ON h.listing_id = l.id
    WHERE h.id = hotel_id AND (l.host_id = (select auth.uid()) OR EXISTS (SELECT 1 FROM profiles WHERE id = (select auth.uid()) AND role IN ('admin', 'super_admin')))
  )
);
...
CREATE POLICY "Hosts and admins can view guesthouses"
ON guesthouses FOR SELECT
TO authenticated
USING (
  EXISTS (SELECT 1 FROM listings WHERE id = listing_id AND (host_id = (select auth.uid()) OR EXISTS (SELECT 1 FROM profiles WHERE id = (select auth.uid()) AND role IN ('admin', 'super_admin'))))
